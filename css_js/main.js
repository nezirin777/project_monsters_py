...
(() => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        document.querySelectorAll(".monster-select").forEach(select => {
            ...
            select.addEventListener("change", () => {
                ...
                const targetImgName =
                    select.getAttribute("data-target");

                const img =
                    form.querySelector(`img[name="${targetImgName}"]`);
    ...
    function loadToastPosition() {
        ...
        const saved =
            localStorage.getItem(TOAST_POSITION_KEY)
            || 'toast-center';

        setToastPosition(saved);

        const select =
            document.getElementById("toast-position-select");
    ...
    function handleUILock(e) {
        ...
        const msg =
            form.dataset.loading || "処理中...";

        const timeoutSeconds =
            parseInt(form.dataset.timeout, 10) || 20;
    ...
    function setUILock(
        ...
        if (locked && overlay) {
            ...
            let msgEl =
                overlay.querySelector(".overlay-text");
    ...
    function showConfirm(message) {
        ...
        return new Promise((resolve) => {
            ...
            if (
                ...
                console.warn(
                    'Confirm modal elements missing.'
                );
