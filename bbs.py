...
# 設定クラス
class BBSConfig:
    DATADIR = conf.Conf["savedir"]
    LOGFILE = Path(DATADIR) / "bbslog.log"
    MAX_LOG_LINES = conf.Conf["max_log_lines"]
    MESSAGE_COLORS = conf.Conf["message_colors"]
    CGI_NAME = conf.Conf["bbs_name"]
...
def render_page(form, csrf_token):
    ...
    template = env.get_template("bbs_tmp.html")
    html = template.render(
        cgi_name=Config.CGI_NAME,
